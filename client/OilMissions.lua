...
function beginningstage()
  Wait(1000)
  VORPcore.NotifyRightTip(_U('FillYourOilWagon'), 4000)
  local fillcoords = CoordRandom(OilWagonTable.FillPoints)

  --Blip and Waypoint setup
  local blip1 = BlipWaypoin(fillcoords.fillpoint.x, fillcoords.fillpoint.y, fillcoords.fillpoint.z, _U('FillBlipName'))
  ...
  VORPcore.NotifyRightTip(_U('FillingOilwagon'), 4000)
  Wait(3000)
  ...
  progressbar.start(_U('FillingOilwagon'), Config.OilWagonFillTime, function()
  end, 'circle')
...
function deliveroil()
  ...
  progressbar.start(_U('UnloadingOil'), Config.OilWagonFillTime, function()
  end, 'circle')
  Wait(Config.OilWagonFillTime)
  if Progressbardeadcheck then
    VORPcore.NotifyRightTip(_U('Missionfailed'), 4000)
  ...
  ClearPedTasksImmediately(createdped)
  FreezeEntityPosition(Createdwagon, false)
  VORPcore.NotifyRightTip(_U('OilDelivered'), 4000)
  VORPcore.NotifyRightTip(_U('ReturnOilWagon'), 4000)

  --------------------This will handle the despawning of the ped, and the return wagon mission---------------------------
  --Waypoint and Blip Setup
  local oilbl = BlipWaypoin(OilWagonTable.WagonSpawnCoords.x, OilWagonTable.WagonSpawnCoords.y, OilWagonTable.WagonSpawnCoords.z, _U('ReturnBlip'))
  ...
  FreezeEntityPosition(Createdwagon, true)
  RemoveBlip(oilbl)
  VORPcore.NotifyRightTip(_U('CollectOilDeliveryPay'), 4000)
...
OilWagonTable.ManagerSpawn = Config.OilManagerNPC --This is where the manager npc will spawn(Do not change!!)
